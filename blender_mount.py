...
def primitive(s):
    if s[0]=='serrated':
        ...
        vertices,faces=serration_mesh(center,axis,height)
        mesh=bpy.data.meshes.new('72 radial tilt serrations')
        mesh.from_pydata(vertices,[],faces);mesh.update()
        obj=bpy.data.objects.new('Tilt serration tool',mesh)
...
def stl(obj,path):
    ...
    rot=Matrix.Rotation(-math.pi/2 if obj.name=='Part_2_SwivelArm' else math.pi/2,4,'Y')
